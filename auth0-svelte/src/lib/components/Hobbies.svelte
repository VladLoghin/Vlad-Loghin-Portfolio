...
<script lang="ts">
	...
	let loading = true;
	let showModal = false;
	let editingItem: Hobby | null = null;
	...
	function openEditModal(item: Hobby) {
		editingItem = item;
	...
	async function handleSubmit() {
		...
		const url = editingItem ? `/api/hobbies/${editingItem.id}` : '/api/hobbies';

		const body = {
			hobbyName: draftTitle.trim(),
			description: draftDescription.trim(),
			tags: draftTagsInput.split(',').map(s => s.trim()).filter(Boolean)
		};
	...
	async function toggleActive(item: Hobby) {
		const token = await getToken();
		if (!token) return;

		try {
			const response = await fetch(
				`/api/hobbies/${item.id}/active?active=${!item.active}`,
				{
					method: 'PATCH',
					headers: { 'Authorization': `Bearer ${token}` }
				}
			);
			if (response.ok) await fetchHobbies();
		} catch (error) {
...
<section id="hobbies" class="section shell snap-section">
	<div class="section-head">
		...
		<p class="lead">Outside of building software, I like creative projects that sharpen taste, iteration, and discipline.</p>

		{#if $isAdmin}
			<button class="btn-new" type="button" on:click={openCreateModal}>+ New Hobby</button>
		{/if}
	</div>

	{#if loading}
		<div class="loading">Loading hobbies...</div>
	{:else if displayItems.length === 0}
		<div class="empty-state">
			{#if $isAdmin}
				<p>No hobbies yet. Click "New Hobby" to create one.</p>
			{:else}
	...
	{:else}
		<div class="carousel-wrapper">
			...
			<div class="hobbies-carousel" bind:this={scrollContainer} on:scroll={checkScroll}>
				{#each displayItems as hobby (hobby.id)}
					<article class="card hobby-card" class:inactive-card={!hobby.active}>
						<h3 class="h3">{hobby.title}</h3>
						<p class="p muted">{hobby.description}</p>
		...
		<div class="modal" role="dialog" aria-modal="true" on:click|stopPropagation>
			<div class="modal-head">
				<h3 class="h3" style="margin:0;">
					{editingItem ? 'Edit Hobby' : 'Add Hobby'}
				</h3>
			...
			<div class="modal-body">
				<label class="label">
					Title
					<input class="input modal-input" bind:value={draftTitle} placeholder="e.g. Music" required />
				</label>

				<label class="label">
					Description
					<textarea class="input modal-input textarea-input" rows="5" bind:value={draftDescription} placeholder="Describe your hobby..."></textarea>
				</label>

				<label class="label">
					Tags (comma separated)
					<input class="input modal-input" bind:value={draftTagsInput} placeholder="e.g. Guitar, Piano" />
				</label>
			</div>
...
<style>
	...
	.hobbies-carousel::-webkit-scrollbar { display: none; }
	...
	@media (max-width: 980px) {
		.hobbies-carousel { padding: 0 calc(50% - 250px); }
		.hobby-card { flex: 0 0 500px; max-width: 500px; }
	}
	@media (max-width: 640px) {
		.hobbies-carousel { padding: 0 20px; }
		.hobby-card { flex: 0 0 calc(100% - 40px); max-width: 100%; }
	}
